...
export function applyAssistantStreamChunk(options: ApplyAssistantStreamChunkOptions): void {
  ...
  if (event && typeof event === 'object' && event.type) {
    ...
    if (!baseMessage) {
      console.warn(`[AssistantPanel] Target message index ${options.targetIdx} does not exist, ignoring event`, event)
      return
    }
    ...
    if (type === 'error') {
      const errMessage = data.error || i18n.global.t('app.assistant.executionFailed')
      applyAgentStreamEvent(baseMessage as any, event as any, {
        trackToolStartInTools: false,
        appendErrorToContent: false,
      })
  ...
  const msg = options.messages.value[options.targetIdx]
  if (!msg) {
    console.warn(`⚠️ [AssistantPanel] Target message index ${options.targetIdx} does not exist, stopping stream output`)
    return
  }
