...
// Calculate total node count and completed node count
const getNodeStats = (run: any) => {
  // Estimate node count from progress percentage
  if (run.progress !== undefined && run.progress > 0) {
    const total = 10 // Assume an average of 10 nodes
...
watch(() => completedRuns.value.length, (newVal, oldVal) => {
  ...
  console.log('[WorkflowStatusBar] completedRuns changed:', oldVal, '->', newVal)
  if (newVal > previous && newVal > lastCompletedCount) {
    console.log('[WorkflowStatusBar] Trigger flash animation')
    isFlashing.value = true
    lastCompletedCount = newVal
    if (flashTimer) clearTimeout(flashTimer)
    flashTimer = setTimeout(() => {
      isFlashing.value = false
      console.log('[WorkflowStatusBar] Flash animation ended')
    }, 2000)
  }
}, { immediate: true })
...
watch(() => activeRuns.value, (runs) => {
  console.log('[WorkflowStatusBar] activeRuns updated:', runs)
}, { deep: true })

watch(() => activeRunCount.value, (count) => {
  console.log('[WorkflowStatusBar] activeRunCount updated:', count)
})
</script>
  ...
  >
    <div @click.capture="(e) => isDragging && e.stopPropagation()">
      <el-popover
        ...
        placement="top-end"
        :title="t('workflow.executionStatus')"
        :width="360"
        trigger="click"
      >
        <template #reference>
           ...
           <div class="status-content">
             <span class="status-text">
               <template v-if="activeRunCount > 0">
                  {{ t('workflow.runsSummary', { running: activeRunCount, completed: completedRuns.length }) }}
               </template>
               <template v-else>
                  {{ t('workflow.workflowReady') }}
               </template>
             </span>
           </div>
        </div>
