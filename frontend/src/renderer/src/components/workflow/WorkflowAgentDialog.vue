    ...
    <div v-if="visible" class="agent-window" :class="{ 'is-collapsed': collapsed }" :style="windowStyle" @mousedown.stop>
      <div class="window-header" @mousedown.stop="handleWindowMouseDown">
        <div class="window-title">
          ...
          <el-tag v-if="!collapsed" size="small" type="info">{{ t('workflow.workflowLabel', { id: props.workflowId || '-' }) }}</el-tag>
        </div>
      ...
      <div v-show="!collapsed" class="window-toolbar">
        ...
        >
          <el-option
            ...
            :value="item.id"
          />
          <el-option v-if="!llmLoading && !llmOptions.length" :label="t('workflow.noModelsAvailable')" :value="-1" disabled />
        </el-select>
        ...
        <el-switch
          ...
          in:active-text="t('workflow.thinking')"
        />

        <el-tag v-if="pendingPatchOps.length" size="small" type="success">{{ t('workflow.opsCount', { count: pendingPatchOps.length }) }}</el-tag>
      </div>
      ...
      <div v-show="!collapsed" class="window-body">
        <agent-message-list
          ...
          :messages="messages"
          :streaming="streaming"
          :empty-description="t('workflow.agentEmptyDesc')"
        />
        ...
        <div class="patch-panel" v-if="pendingPatchOps.length || pendingDiff">
          <div class="patch-title-row">
            <div class="patch-title-meta">
              ...
              <el-tag size="small" type="success">{{ t('workflow.opsCount', { count: pendingPatchOps.length }) }}</el-tag>
            </div>
        ...
        <div class="agent-footer">
          <AgentComposer
            ...
            :rows="3"
            resize="none"
            :placeholder="t('workflow.agentComposerPlaceholder')"
            :disabled="streaming"
  ...
  try {
    ...
    if (!result.success) {
      ...
      if (result.error === 'revision_mismatch') {
        lastPatchError.value = t('workflow.codeUpdatedSyncRetry')
      } else {
        lastPatchError.value = result.error || (result.validation?.is_valid ? '' : t('workflow.validationNotPassed'))
      }
    } else {
      lastPatchError.value = ''
    }
    ...
    if (detail?.code === 'revision_mismatch') {
      ...
      lastPatchError.value = detail?.message || detail || error?.message || t('workflow.patchPreviewFailed')
    }
  }
}
  ...
  try {
    ...
    if (!result.success) {
      lastPatchError.value = result.error === 'revision_mismatch'
        ? t('workflow.codeChangedRetry')
        : (result.error || (result.validation?.is_valid ? '' : t('workflow.validationNotPassed')))
    } else {
      lastPatchError.value = ''
    }
  } catch (error: any) {
    ...
    const detail = error?.response?.data?.detail
    if (detail?.code === 'revision_mismatch') {
      lastPatchError.value = t('workflow.codeChangedRetry')
    } else {
      lastPatchError.value = detail?.message || detail || error?.message || t('workflow.patchPreviewFailed')
    }
  }
...
async function confirmApplyPatch() {
  ...
  try {
    await ElMessageBox.confirm(t('workflow.confirmApplyPatch'), t('workflow.applyPatch'), {
      type: 'warning',
      confirmButtonText: t('workflow.confirmApply'),
  ...
  try {
    ...
    if (!result.success) {
      lastValidationFailed.value = !Boolean(result.validation?.is_valid)
      lastPatchError.value = result.error || t('workflow.patchApplyFailed')
    ...
    ElMessage.success(t('workflow.patchApplySuccess'))
    clearPendingPatch()
  } catch (error: any) {
    const detail = error?.response?.data?.detail
    if (detail?.code === 'revision_mismatch') {
      ...
      lastPatchError.value = t('workflow.codeUpdatedRetryApply')
      void previewPatchDryRunOnce()
    } else {
      lastPatchError.value = detail?.message || detail || error?.message || t('workflow.patchApplyFailed')
    }
    ElMessage.error(lastPatchError.value)
  } finally {
    applyingPatch.value = false
...
function sendMessage() {
  ...
  const streamHandle = workflowAgentChatStreaming(
        ...
        onToolEnd: (toolName, result) => {
          if (toolName === 'wf_apply_patch' && result && typeof result === 'object') {
            ...
            if (ops.length && hasValidReplaceCode) {
              ...
              lastPatchError.value = patchResult?.message || t('workflow.invalidPatchMissingNewCode')
            }
            ...
            if (patchResult.error) {
              lastPatchError.value = patchResult.error === 'revision_mismatch'
                ? t('workflow.codeVersionChangedResynced')
          ...
          if (toolName === 'wf_replace_code' && result && typeof result === 'object') {
            ...
            if (generatedCode.trim()) {
              ...
              lastPatchError.value = replaceResult?.message || t('workflow.replaceCodeMissingNewCode')
            }
    ...
    err => {
      streaming.value = false
      streamCancelRef.value = null
      ElMessage.error(err?.message || t('workflow.agentRequestFailed'))
    },
  )
...
function suggestFixForValidation() {
  ...
  draft.value = t('workflow.suggestFixDraft')
}
...
async function loadLlmOptions() {
  ...
  try {
    ...
    if (!llmOptions.value.length) {
      ElMessage.warning(t('workflow.noModelsConfigureFirst'))
    }
    ...
    llmOptions.value = []
    ElMessage.error(error?.message || t('workflow.loadModelsFailed'))
  } finally {
