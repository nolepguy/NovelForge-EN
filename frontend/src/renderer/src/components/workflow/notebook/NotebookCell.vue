    ...
    <div class="cell-header">
      ...
      <div class="cell-progress" v-if="cell.status === 'progress'">
        <el-progress
          ...
          :format="() => cell.message || t('misc.processing')"
          :stroke-width="6"
        />
      </div>
    </div>
      ...
      <div class="cell-output" v-if="hasOutput">
        ...
        <div v-if="cell.status === 'completed'" class="output-success">
          <div class="output-header">
            ...
            <span>{{ t('misc.executionSuccess') }}</span>
          </div>
        ...
        <div v-if="cell.status === 'error'" class="output-error">
          <div class="output-header">
            ...
            <span>{{ t('misc.executionFailed') }}</span>
          </div>
          <div class="output-content">
            <pre>{{ cell.error }}</pre>
          </div>
        ...
        <div v-if="cell.status === 'progress'" class="output-progress">
          ...
          <span>{{ cell.message || t('misc.processing') }}</span>
        </div>
      </div>
    </div>
  </div>
...
const statusText = computed(() => {
  const textMap = {
    running: t('misc.statusRunning'),
    progress: t('misc.statusProcessing'),
    completed: t('misc.statusCompleted'),
    error: t('misc.statusFailed')
  }
  return textMap[props.cell.status] || t('misc.statusUnknown')
})
