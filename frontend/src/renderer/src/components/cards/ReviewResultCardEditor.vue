    ...
    <div class="review-header">
      <div class="review-header-main">
        ...
        <div class="review-actions">
          <el-button size="small" plain type="primary" @click="jumpToTarget">{{ t('misc.jumpToReviewedCard') }}</el-button>
        </div>
      </div>
      <div class="review-target">
        {{ t('misc.reviewTarget', { name: targetTitle || t('misc.unnamedTarget') }) }}
      </div>
    </div>
...
const reviewMarkdown = computed(() => String(content.value.review_markdown || t('misc.noContent')))
...
const verdictLabel = computed(() => {
  switch (content.value.quality_gate) {
    case 'pass':
      return t('misc.verdictPass')
    case 'block':
      return t('misc.verdictBlock')
    default:
      return t('misc.verdictRevise')
  }
})
