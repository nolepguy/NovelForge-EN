  ...
  <div class="editor-header">
    <div class="header-main">
      <div class="left">
        ...
        <span v-if="lastSavedAt" class="last-saved">{{ t('misc.lastSaved', { time: lastSavedAt }) }}</span>
      </div>
      <div class="right">
        <div class="context-action-combo">
          <el-tooltip :content="t('misc.openContextDrawer')">
            <el-button type="primary" plain class="context-main-button" @click="$emit('open-context')">
              {{ t('misc.contextInjection') }}
          ...
          <el-popover v-model:visible="slotPickerVisible" trigger="click" width="220" popper-class="context-slot-popper">
            <template #reference>
              <el-button type="primary" plain class="context-trigger-button" :title="t('misc.switchContextSlot')">
        ...
        <el-button v-if="!isChapterContent" type="success" plain @click="$emit('generate')">{{ t('misc.aiGenerate') }}</el-button>
        <el-button 
          ...
          @click="$emit('save')"
        >
          {{ needsConfirmation ? t('misc.confirmAndSave') : t('common.save') }}
        </el-button>
        <el-dropdown>
          <el-button text>{{ t('common.more') }}</el-button>
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="$emit('open-versions')">{{ t('misc.historyVersions') }}</el-dropdown-item>
...
const props = defineProps<{
  ...
  needsConfirmation?: boolean  // AI changes require confirmation
  activeContextTemplateKind?: ContextTemplateKind
}>()

// Compute whether saving is allowed: if confirmation is required, saving is allowed even without changes
...
const statusTag = computed(() => {
  if (props.needsConfirmation) return { type: 'warning', label: t('misc.aiModified') }
  if (props.saving) return { type: 'warning', label: t('misc.savingStatus') }
  if (props.dirty) return { type: 'info', label: t('misc.unsavedStatus') }
  return { type: 'success', label: t('common.saved') }
})
...
.right :deep(.el-button) { white-space: nowrap; }
