export interface BuilderField {
  ...
  description?: string
  example?: string
  // tuple element types (only used by Builder for display and generating simple tuples)
  tupleItems?: Array<'string' | 'number' | 'integer' | 'boolean'>
  // Whether to exclude from the AI-generated valid schema
  aiExclude?: boolean
}
...
export function schemaToBuilder(schema: any): BuilderField[] {
  ...
  // Allow $defs, but this function only parses the $ref name from main properties → relation
  for (const key of Object.keys(props)) {
    ...
    // examples[0] takes precedence, otherwise example
...
export function builderToSchema(fields: BuilderField[]): any {
  ...
  // defs are collected/injected by the caller (SchemaStudio) so they can be reused across models
  return schema
} 
