...
def apply_patch_ops(code: str, patch_ops: Sequence[WorkflowPatchOp]) -> Tuple[str, List[str], int]:
    ...
    for op in patch_ops:
        ...
        if op_name == "replace_code":
            ...
            if not full_code:
                raise ValueError("replace_code is missing new_code")
        ...
        if op_name in ("insert_node_before", "insert_node_after"):
            if not op.target_node or not op.new_block:
                raise ValueError(f"{op_name} is missing target_node or new_block")
            target = next((b for b in blocks if b.variable == op.target_node), None)
            if not target:
                raise ValueError(f"Target node not found: {op.target_node}")
        ...
        if op_name == "delete_node":
            if not op.target_node:
                raise ValueError("delete_node is missing target_node")
            target = next((b for b in blocks if b.variable == op.target_node), None)
            if not target:
                raise ValueError(f"Target node not found: {op.target_node}")
        ...
        if op_name == "update_node_meta":
            if not op.target_node:
                raise ValueError("update_node_meta is missing target_node")
            target = next((b for b in blocks if b.variable == op.target_node), None)
            if not target:
                raise ValueError(f"Target node not found: {op.target_node}")
        ...
        if op_name == "update_node_call":
            if not op.target_node or not op.new_call:
                raise ValueError("update_node_call is missing target_node or new_call")
            target = next((b for b in blocks if b.variable == op.target_node), None)
            if not target:
                raise ValueError(f"Target node not found: {op.target_node}")
            if target.assignment_line < 0:
                raise ValueError(f"Node is missing an assignment statement: {op.target_node}")
            ...
            while call_lines and not call_lines[-1].strip():
                call_lines.pop()
            if not call_lines:
                raise ValueError("update_node_call new_call is empty")
        ...
        if op_name == "rename_variable":
            if not op.old_name or not op.new_name:
                raise ValueError("rename_variable is missing old_name or new_name")
