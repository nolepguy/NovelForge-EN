...
class StructuredCardMemoryExtractor:
    ...
    async def extract(
        ...
        *,
        ...
        if not prompt:
            raise ValueError(f"Prompt not found: {self.prompt_name}")
        ...
        ref_blocks: list[str] = []
        if extra_context:
            ref_blocks.append(f"[Supplementary context, for reference only, do not mechanically repeat]\n{extra_context}")
        ...
        user_prompt = (
            f"{ref_text}"
            f"Participating entity info: {participant_desc}\n\n"
            f"Chapter text is as follows:\n{text}\n"
        )
        ...
        if not isinstance(result, self.output_model):
            raise ValueError(f"{self.name} failed: output format does not match {self.output_model.__name__}")
    ...
    def persist(
        ...
        *,
        ...
        card_type = session.exec(select(CardType).where(CardType.name == self.spec.card_type_name)).first()
        if not card_type:
            raise ValueError(f"Card type not found: {self.spec.card_type_name}")
