...
_SPEC = StructuredCardExtractorSpec(
    ...
    name="Organization State Extraction",
    prompt_name="Organization State Extraction",
    card_type_name="Organization Card",
    ...
    reference_title="Existing Organization Card Reference",
)
...
class OrganizationStateExtractor(StructuredCardMemoryExtractor):
    ...
    def build_reference_lines(self, model: OrganizationCard) -> list[str]:
        return [
            ...
            f"  Current influence: {model.influence or 'Not filled'}",
            f"  External relations: {'；'.join(model.relationship or []) or 'None'}",
