...
_SPEC = StructuredCardExtractorSpec(
    code="concept_state",
    name="Concept Mastery Extraction",
    prompt_name="Concept Mastery Extraction",
    card_type_name="Concept Card",
    ...
    reference_title="Existing Concept Card Reference",
)
...
class ConceptStateExtractor(StructuredCardMemoryExtractor):
    ...
    def build_reference_lines(self, model: ConceptCard) -> list[str]:
        return [
            f"- {model.name}",
            f"  Category: {model.category or 'Not filled'}",
            f"  Rule: {model.rule_definition or 'Not filled'}",
            f"  Mastery hint: {model.mastery_hint or 'Not filled'}",
        ]
