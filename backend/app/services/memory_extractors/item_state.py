...
_SPEC = StructuredCardExtractorSpec(
    code="item_state",
    name="Item State Extraction",
    prompt_name="Item State Extraction",
    card_type_name="Item Card",
    ...
    reference_title="Existing Item Card Reference",
)
...
class ItemStateExtractor(StructuredCardMemoryExtractor):
    ...
    def build_reference_lines(self, model: ItemCard) -> list[str]:
        return [
            ...
            f"  Current state: {model.current_state or 'Not filled'}",
            f"  Effect: {model.power_or_effect or 'Not filled'}",
        ]
