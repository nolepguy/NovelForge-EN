...
class CardExportService:
    """Project card export service (scope filtering + format serialization)."""
    ...
    def export(self, project_id: int, request: CardExportRequest) -> CardExportPayload:
        ...
        if not cards:
            raise BusinessException("No cards available for export under the current conditions", status_code=404)
    ...
    def _load_cards(self, project_id: int, request: CardExportRequest) -> List[Card]:
        ...
        if request.scope == "single":
            if request.card_id is None:
                raise BusinessException("scope=single is missing card_id", status_code=400)
            card = next((item for item in ordered_cards if item.id == request.card_id), None)
            if not card:
                raise BusinessException("Target card does not exist or does not belong to the current project", status_code=404)
            return [card]

        if request.scope == "type":
            if request.card_type_id is None:
                raise BusinessException("scope=type is missing card_type_id", status_code=400)
            card_type = self.db.get(CardType, request.card_type_id)
            if not card_type:
                raise BusinessException("Card type does not exist", status_code=404)
    ...
    def _build_text(
        ...
        lines: List[str] = [
            ...
            f"Export scope: {self._scope_text(request, cards)}",
        ...
        for index, card in enumerate(cards, start=1):
            lines.extend(
                [
                    "=" * 72,
                    f"[{index}] {card.title}",
                    f"Type: {self._card_type_name(card)}",
                    f"ID: {card.id}",
                    f"Parent ID: {card.parent_id}",
                    f"Created at: {card.created_at.isoformat() if card.created_at else ''}",
                    "-" * 72,
    ...
    def _build_markdown(
        ...
        for index, card in enumerate(cards, start=1):
            ...
            lines.append(f"- Type: {self._card_type_name(card)}")
            lines.append(f"- ID：{card.id}")
            lines.append(f"- Parent ID: {card.parent_id}")
            lines.append(f"- Created at：{card.created_at.isoformat() if card.created_at else ''}")
            lines.append("")
    ...
    def _scope_text(self, request: CardExportRequest, cards: List[Card]) -> str:
        ...
        if request.scope == "single":
            if cards:
                return f"Single card ({cards[0].title}）"
            return "Single card"
        if request.scope == "type":
            if cards:
                return f"Type cards ({self._card_type_name(cards[0])}）"
            return "Type cards"
