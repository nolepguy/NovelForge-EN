...
REVIEW_RESULT_FOLDER_TITLE = "Review Results"
...
def _build_system_prompt(session: Session, prompt_name: str) -> str:
    ...
    if not prompt or not prompt.template:
        raise HTTPException(status_code=400, detail=f"Prompt name not found: {prompt_name}")
...
def _get_review_card_type_or_500(session: Session) -> CardType:
    ...
    if not card_type:
        raise HTTPException(status_code=500, detail=f"Missing card type: {REVIEW_RESULT_CARD_TYPE_NAME}")
    return card_type
...
def _resolve_review_card_title(target_title: str) -> str:
    return f"{(target_title or 'Untitled target').strip() or 'Untitled target'} · Review Results"
...
def delete_review_result_card(session: Session, review_card_id: int) -> bool:
    ...
    if card.card_type_id != review_card_type.id:
        raise HTTPException(status_code=400, detail="Target card is not a Review Result card")
    session.delete(card)
    session.commit()
    return True
