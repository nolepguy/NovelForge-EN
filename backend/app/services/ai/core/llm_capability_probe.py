    ...
    try:
        ...
        response = await model.ainvoke(
            [
                ...
                HumanMessage(content="This is a piece of test text."),
            ]
        )
        if not _message_text(response):
            raise ValueError("Empty response")
...
def _summary(overall: str, results: dict[str, ProbeResult], recommended: LLMCapabilityRecommendedMode) -> str:
    if overall == "unusable":
        if results["models_list"].status == "pass" and results["basic_chat"].status == "fail":
            return "Model list is available, but the chat request was blocked. You may try a compatibility fix."
        return "Basic connection failed; the current config is unusable."
    if overall == "full":
        return "Basic chat, review, and native tool calling are available."
    if overall == "react_assistant":
        return "Native tools are unavailable, but ReAct tool mode works; recommend using ReAct for the inspiration assistant."
    if overall == "writing_review_only":
        return "The model is suitable for plain writing and review; using the standard tool assistant is not recommended."
    if recommended.get("disable_stream"):
        return "Basic chat is available, but streaming output failed; recommend disabling streaming."
    return "The model only passed the basic chat check."
...
async def run_capability_test(request: LLMCapabilityTestRequest) -> dict[str, Any]:
    ...
    if request.try_repair and results["basic_chat"].status == "fail":
        repair_notes.append(f"User-Agent=current config basic connection failed: {results['basic_chat'].message}")
        ...
        for candidate in REPAIR_USER_AGENT_CANDIDATES:
            ...
            if ua_basic.status == "pass":
                repair_notes.append(f"User-Agent={candidate} repair succeeded")
                ...
                break
            repair_notes.append(f"User-Agent={candidate} basic connection failed: {ua_basic.message}")
    ...
    if repair_notes:
        tags.extend([f"Repair attempt: {note}" for note in repair_notes])
