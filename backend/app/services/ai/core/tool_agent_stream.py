    ...
    if not accumulated_text.strip() and not reasoning_accumulated.strip():
        if tool_end_count > 0:
            if tool_end_failed_count == tool_end_count:
                fallback_text = "Tool calls were executed, but all tool results failed. Please review the tool results, adjust, and retry."
            else:
                fallback_text = "Tool calls were executed. Please review the tool results."
        else:
            fallback_text = "No visible reply text was produced this round. Please retry or adjust your question."
