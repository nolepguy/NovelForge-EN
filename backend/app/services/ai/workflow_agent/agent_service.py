...
WORKFLOW_AGENT_PROMPT_NAME = "Workflow Agent"
WORKFLOW_AGENT_REACT_PROMPT_NAME = "Workflow Agent - React"
...
def _append_node_briefs_to_prompt(base_prompt: str) -> str:
    ...
    return (
        f"{base_prompt}\n\n"
        "【System built-in node brief list (injected at startup)】\n"
        "You already have brief information for all nodes. Prefer selecting nodes directly from this brief list, and only call wf_get_node_metadata when you need field-level details.\n"
        f"{node_map}"
    )
...
def get_workflow_agent_system_prompt(session: Session, react_enabled: bool = False) -> str:
    ...
    try:
        ...
        if prompt and prompt.template and prompt.template.strip():
            ...
            return _append_node_briefs_to_prompt(prompt.template.strip())
        raise RuntimeError(
            f"Prompt not found: {WORKFLOW_AGENT_PROMPT_NAME}. "
            "Please run bootstrap initialization first, or create this prompt in prompt management."
        )
    except Exception as exc:
        logger.error("[WorkflowAgent] load prompt from DB failed: {}", exc)
        raise RuntimeError(
            f"Failed to load prompt: {WORKFLOW_AGENT_PROMPT_NAME}. "
            "Please check the database and bootstrap status."
        ) from exc
...
def _build_user_prompt(request: WorkflowAgentChatRequest) -> str:
    ...
    if pending_code_text:
        pending_code_section = (
            "\n\nUnapplied patch candidate code (prefer continuing modifications based on this; do not first revert to the current DB code):\n"
            f"```python\n{pending_code_text}\n```"
        )

    if user_prompt:
        return f"{header}\n\nUser request:\n{user_prompt}{pending_code_section}"
    return f"{header}\n\nThe user request is empty; please first clarify the requirements and read the current code."
...
async def stream_workflow_agent_chat(
    ...
    for attempt in range(WORKFLOW_AGENT_MAX_VALIDATE_RETRIES + 1):
            ...
            if patch_result is not None:
                ...
                if patch_failed:
                    ...
                    if validation_errors:
                        ...
                        retry_hint = "Validation did not pass"
                else:
                    retry_hint = ""
        ...
        latest_prompt = (
            "The previous patch did not pass validation; please continue fixing and call the tool again to produce a patch that passes validation.\n"
            f"Failure reason: {retry_hint}"
        )
