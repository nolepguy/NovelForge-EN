...
- 若干功能优化
  - **LLM 配置页能力检测**
    - 可在 LLM 配置页触发模型能力/兼容性测试，辅助判断基础对话、流式、结构化输出、工具调用等兼容情况

  - **灵感助手批量正文修改建议**
    - 灵感助手可一次返回多条正文修改建议，编辑器支持逐条查看、接受或拒绝
    - 保留工具结果处理和文本格式降级解析，降低模型返回格式不稳定时的失败概率

  - **任务完成提醒**
    - 助手回复或编辑建议生成完成后，可触发提示音和桌面通知（默认关闭，可在助手设置中开关）
- bug修复
